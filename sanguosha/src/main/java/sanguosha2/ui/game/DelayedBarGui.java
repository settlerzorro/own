	...
	private static class DelayedIcon extends JPanel {
		...
		DelayedIcon(DelayedStackItem item) {
			...
			try {
				switch(item.type) {
					case LIGHTNING:
						this.icon = ImageIO.read(getClass().getResource("cards_img/Lightning_marker.png"));
						break;
					case OBLIVION:
						this.icon = ImageIO.read(getClass().getResource("cards_img/Oblivion_marker.png"));
						break;
					case STARVATION:
						this.icon = ImageIO.read(getClass().getResource("cards_img/Starvation_marker.png"));
						break;
				}
			}  catch (IOException e) {
